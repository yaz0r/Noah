...
s16 findTriangleInWalkMesh(int posX, int posZ, int walkmeshId, SVECTOR* position, VECTOR* triangleNormal)
{
    ...
    for (int i = 0; i < g_walkMeshNumTrianglePerBlock[walkmeshId]; i++)
    {
        ...
        if ((NormalClip(pos0, pos1, refPos) > -1) && 
            (NormalClip(pos1, pos2, refPos) > -1) &&
            (NormalClip(pos2, pos0, refPos) > -1) )
        {
...
void resetFieldScriptEntityValues(int index)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        pFieldScriptEntity->m8C_scriptSlots[i].m4_flags._30000_status = 0;
        pFieldScriptEntity->m8C_scriptSlots[i].m4_flags._3C0000 = 0xF;
        pFieldScriptEntity->m8C_scriptSlots[i].m4_flags._400000 = 0;
        pFieldScriptEntity->m8C_scriptSlots[i].m4_flags._C00000_walkMode = 0;
        pFieldScriptEntity->m8C_scriptSlots[i].m4_flags._FFFF_numSteps = -1;
    }
...
void emptyPartySlot(int param_1)
{
    ...
    if (fieldEntity != 0xff) {
        ...
        pCurrentFieldScriptActor->m0_fieldScriptFlags._1_updateScriptDisabled = 1;
...
int spriteWalkToPositionOrActor(int param_1)
{
    ...
    switch (pCurrentFieldScriptActor->m8C_scriptSlots[pCurrentFieldScriptActor->mCE_currentScriptSlot].m4_flags._C00000_walkMode)
    {
    case 0:
    ...
    if ((pCurrentFieldScriptActor->m8C_scriptSlots[pCurrentFieldScriptActor->mCE_currentScriptSlot].m4_flags._FFFF_numSteps == 0) ||
        (distanceToDestination <= (stepLength + 1 + collisionRadius)))
    {
        ...
        pCurrentFieldScriptActor->m8C_scriptSlots[pCurrentFieldScriptActor->mCE_currentScriptSlot].m4_flags._FFFF_numSteps = -1;
        pCurrentFieldScriptActor->m8C_scriptSlots[pCurrentFieldScriptActor->mCE_currentScriptSlot].m4_flags._C00000_walkMode = 0;
        pCurrentFieldScriptActor->m0_fieldScriptFlags._800_isJumping = 0;
    ...
    {
        pCurrentFieldScriptActor->m8C_scriptSlots[pCurrentFieldScriptActor->mCE_currentScriptSlot].m4_flags._FFFF_numSteps--;
        pCurrentFieldScriptActor->m106_currentRotation = pCurrentFieldScriptActor->m104_rotation = fp_atan2(travelDelta);
...
void exectueEntitiesUpdateFunction()
{
    ...
    for (int i = 0; i < numEntitiesToUpdate; i++)
    {
        ...
        if (((pFieldEntity->m58_flags & 0xf00) != 0) && ((pFieldEntity->m4C_scriptEntity->m4_flags.m_rawFlags & 0x100000) == 0))
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                if (pCurrentFieldScriptActor->m8C_scriptSlots[j].m4_flags._3C0000 <= maxScriptIndex)
                {
                    pCurrentFieldScriptActor->mCE_currentScriptSlot = j;
                    maxScriptIndex = pCurrentFieldScriptActor->m8C_scriptSlots[j].m4_flags._3C0000;
                }
            }
            if (maxScriptIndex == 0xF)
            {
                ...
                pCurrentFieldScriptActor->m8C_scriptSlots[0].m4_flags._3C0000 = 7;
            }
            ...
            if (!(pCurrentFieldScriptActor->m0_fieldScriptFlags._1_updateScriptDisabled))
            {
...
void updateEntityEventCode2(int index, sFieldEntity* pFieldEntity, sFieldScriptEntity* pFieldScriptEntity)
{
    ...
    if (!pFieldScriptEntity->m0_fieldScriptFlags._800_isJumping && ((pFieldScriptEntity->m0_fieldScriptFlags.m_rawFlags & 0x41000) == 0))
    {
...
void setVisualAnimation(sSpriteActor* param_1, int animationId, sFieldEntity* param_3)
{
    if ((param_3->m58_flags & 0x40) != 0) {
        if ((animationId != 3) && (updateEntityEventCode4Var0 == 0)) {
            param_3->m4C_scriptEntity->m0_fieldScriptFlags._800_isJumping = 0;
        }
        if (animationId == 0xff) {
            animationId = 0;
        }
...
void updateEntityEventCode3(int index, sFieldEntity* pFieldEntity, sFieldScriptEntity* pFieldScriptEntity)
{
    ...
    int walkSpeed = 1;
    if (((pFieldScriptEntity->m0_fieldScriptFlags._4000_isPlayerControlled) && ((padButtonForScripts[0].m0_buttons & controllerButtons::CROSS) != 0)) && (playerCanRun == 1)) {
        walkSpeed = 2; // run
    }
    ...
    if (!pFieldScriptEntity->m0_fieldScriptFlags._800_isJumping) {
...
void  startScriptsForCollisions(uint playerEntityIndex, sFieldEntity* pPlayerEntity, sFieldScriptEntity* pPlayerScriptEntity)
{
    ...
    for (int actorId =0; actorId <g_totalActors; actorId++)
    {
        ...
        if (((pTestedScriptEntity->m0_fieldScriptFlags._1_updateScriptDisabled) == 0) && (pPlayerScriptEntity->m74_touchedActor != actorId)) {
        ...
        if (scriptIndexToStart != -1) {
            ...
            if (foundSlot == 8) // script wasn't running already, start it
            {
                ...
                {
                    if (((pTestedScriptEntity->m8C_scriptSlots[slotId].m4_flags._3C0000) == 0xf) && ((pTestedScriptEntity->m8C_scriptSlots[slotId].m4_flags._400000) == 0)) {
                        ...
                        pTestedScriptEntity->m8C_scriptSlots[slotId].m4_flags._3C0000 = newValueInFlags;
...
struct sEntityMoveCheck0Sub1 {
    std::array<sGTE_XY, 4> m0_gteXY;
    sGTE_XY m10;
    std::array<SVECTOR, 4> m14;
    SVECTOR m34;
    long m3C_clipFlag;
    MATRIX m40;
    MATRIX m60;
    MATRIX m80;
    s32 mA0_smallestY;
    std::vector<SVECTOR>::iterator mA4;
    u32 mAC;
    SVECTOR mB0;
};

void validate(u32 psxBase, const sEntityMoveCheck0Sub1& data) {
    validate(psxBase + 0, data.m0_gteXY);
    validate(psxBase + 0x10, data.m10);
    validate(psxBase + 0x14, data.m14);
    validate(psxBase + 0x34, data.m34);
    validate(psxBase + 0x3C, data.m3C_clipFlag);
    validate(psxBase + 0x40, data.m40);
    validate(psxBase + 0x60, data.m60);
    validate(psxBase + 0x80, data.m80);
    validate(psxBase + 0xA0, data.mA0_smallestY);
    validate(psxBase + 0xAC, data.mAC);
    validate(psxBase + 0xB0, data.mB0);
}

int CheckCollisionWithMesh(int actorId, sModelBlock* param_2, int stepX, int stepZ, int* outputY, VECTOR* outputNormal)
{
    sEntityMoveCheck0Sub1* pScratchData = allocateScratchBufferLocationForMoveCheck<sEntityMoveCheck0Sub1>();
    pScratchData->mA0_smallestY = 0x7fffffff;
    pScratchData->mA4 = param_2->m8_vertices.begin();
    ...
    if (attachementMode == 0) {
        ...
        if (actorArray[actorId].m4C_scriptEntity->m75_parentActorId == -1) {
            m3 = &computeProjectionMatrixTempMatrix2;
        ...
        CompMatrix(m3, m1, m2);
        m3 = &actorArray[actorId].mC_matrix;
    }
    else {
        ...
        m3 = &pScratchData->m60;
        createRotationMatrix(&pScratchData->mB0, m3);
        MulRotationMatrix(&actorArray[actorId].mC_matrix, m3);
        pScratchData->m60.t = actorArray[actorId].mC_matrix.t;
        m2 = &computeProjectionMatrixTempMatrix2;
    }

    CompMatrix(m2, m3, &pScratchData->m40);
    SetRotMatrix(&pScratchData->m40);
    SetTransMatrix(&pScratchData->m40);

    VALIDATE_FIELD(FCT_Base, 0x80083564);

    for (int i = 0; i < param_2->m6_numMeshBlock; i++) {
        sModelBlock::sMeshBlock& pModelBlock = param_2->m10_meshBlocks[i];
        u8 primType = pModelBlock.m0_primType;
        u32 numPrims = pModelBlock.m2_primCount;
        pScratchData->mAC = primType;
        if (primType == 0xC4 || primType == 0xC8) {
            continue;
        }
        
        if (!(pModelBlock.m0_primType & 8)) {
            // Triangles
            assert(0);
        }
        else {
            ...
            for (int i = 0; i < numPrims; i++) {
                RotTransSV(&pScratchData->mA4[pModelBlock.m4_indices[i][0]], &pScratchData->m14[0], &pScratchData->m3C_clipFlag);
                RotTransSV(&pScratchData->mA4[pModelBlock.m4_indices[i][1]], &pScratchData->m14[1], &pScratchData->m3C_clipFlag);
                RotTransSV(&pScratchData->mA4[pModelBlock.m4_indices[i][2]], &pScratchData->m14[2], &pScratchData->m3C_clipFlag);
                RotTransSV(&pScratchData->mA4[pModelBlock.m4_indices[i][3]], &pScratchData->m14[3], &pScratchData->m3C_clipFlag);
                pScratchData->m0_gteXY[0].set(pScratchData->m14[0].vx, pScratchData->m14[0].vz);
                pScratchData->m0_gteXY[1].set(pScratchData->m14[1].vx, pScratchData->m14[1].vz);
                pScratchData->m0_gteXY[2].set(pScratchData->m14[2].vx, pScratchData->m14[2].vz);
                pScratchData->m0_gteXY[3].set(pScratchData->m14[3].vx, pScratchData->m14[3].vz);

                // Are we in the quad?
                if ((NormalClip(pScratchData->m0_gteXY[0], pScratchData->m0_gteXY[1], pScratchData->m10) > -1) &&
                    (NormalClip(pScratchData->m0_gteXY[1], pScratchData->m0_gteXY[3], pScratchData->m10) > -1) &&
                    (NormalClip(pScratchData->m0_gteXY[3], pScratchData->m0_gteXY[2], pScratchData->m10) > -1) &&
                    (NormalClip(pScratchData->m0_gteXY[2], pScratchData->m0_gteXY[0], pScratchData->m10) > -1) &&
                    (NormalClip(pScratchData->m0_gteXY[0], pScratchData->m0_gteXY[1], pScratchData->m10) > -1) // that last test is a duplicate of the 1st and is technically useless
                    ) {
                    pScratchData->m34.vx = stepX;
                    pScratchData->m34.vz = stepZ;

                    SVECTOR* pSVar7 = &pScratchData->m14[1];
...
if (pScratchData->m34.vy < pScratchData->mA0_smallestY) {
    ...
    VALIDATE_FIELD(FCT_Base, 0x80083930);
    VALIDATE_REG(FCT_Base, A0, pScratchData->mA0_smallestY);
    if (pScratchData->mA0_smallestY == 0x7fffffff) {
        return -1;
    }
    *outputY = pScratchData->mA0_smallestY;
    return 0;
}
...
void EntityMoveCheck0(uint playerEntityIndex, sFieldEntity* pPlayerEntity, sFieldScriptEntity* pPlayerScriptEntity)
{
    ...
    for (int actorId = 0; actorId < g_totalActors; actorId++)
    {
        ...
        if (entityFlags._1_updateScriptDisabled != 0)
            continue;
        ...
        if (pCurrentFieldScriptEntity->m4_flags.m_rawFlags & 0x80)
        {
            ...
            (pPlayerScriptEntity->m50_surfaceNormal).vx = surfaceNormal[0];
    ...
    if ((actorArray[playerEntityIndex].m4_pVramSpriteSheet)->m7C->mC == 1) {
        resetInputs();
        pPlayerScriptEntity->m0_fieldScriptFlags._800_isJumping = 0;
    }
                ...
                )
            {
                ...
                if (EntityMoveCheck1(i, 0x7fffffff, &actorArray[i], pFieldScriptEntity, 0)) {
                    if (actorArray[i].m4_pVramSpriteSheet->m7C->mC == 1) {
                        ...
                        pFieldScriptEntity->m0_fieldScriptFlags._800_isJumping = 0;
                    }
                }
            }
        }
...
int isFieldBattlePrevented(void)
{
    ...
    if ((bBattleSequenceInitialized == 1) && (updateEntityEventCode4Var0 == 0)) {
        iVar1 = -(s32)(((actorArray[g_playerControlledActor].m4C_scriptEntity)->m0_fieldScriptFlags._800_isJumping) != 0);
    }
    return iVar1;
}
