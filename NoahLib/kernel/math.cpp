...
struct mathTest
{
	...
	{
		{
			VECTOR a = { 0xC4F, 0x0, 0xa3c };
			VECTOR b = { 0x1006, 0, 0x26 };
			VECTOR c;
			OuterProduct12(&a, &b, &c);
			assert(c.vx == 0);
			assert(c.vy == 0xa22);
			assert(c.vz == 0);
		}
		{
			...
			VECTOR a1 = { 0,0,0 };
			VECTOR a2 = { 0,0,0 };
			VECTOR a3 = { 0, 0x10000000, 0 };
			computeMatrix(&a0, &a1, &a2, &a3);

			MATRIX refMatrix;
			refMatrix.m[0] = { 0x1000, 0, 0 };
			refMatrix.m[1] = { 0x0, 0x1000, 0 };
			refMatrix.m[2] = { 0x0, 0, 0x1000};

			//assert(a0.m == refMatrix.m);
		}
	}
} gMathTest;
