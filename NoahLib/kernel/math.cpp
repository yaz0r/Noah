...
const std::array<s16, 202 + 192> SQRT = {
...
const std::array<s16, 202 + 192>::const_iterator ISQRT = SQRT.begin() + 202;
...
s32 SquareRoot0(s32 value)
{
	...
	if (uVar1 != 0x20) {
		...
		return (uint)((int)SQRT[iVar2 + -0x40] << ((int)(0x1f - uVar1) >> 1 & 0x1fU)) >> 0xc;
	}
	return 0;
}
...
void lookAtDivided(MATRIX* pOutputMatrix, VECTOR* vEye, VECTOR* vAt, VECTOR* vUp)
{
	VECTOR local_60 = {
		(vAt->vx - vEye->vx) >> 16,
		(vAt->vy - vEye->vy) >> 16,
...
void VectorNormalInner(VECTOR* input, VECTOR* output)
{
	...
	setCopReg(2, 0x4000, ISQRT[iVar2 + -0x40]); // this can underflow back into SQRT
...
{
    VECTOR tempOutput;
    VectorNormalInner(input, &tempOutput);
