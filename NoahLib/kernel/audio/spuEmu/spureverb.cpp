		...
		sound_hz(hz)
{
	...
	{
		for (int f=0; f<4; f++) {
			y[c][f]=std::make_unique<signed short []>(maxdelay);
		}
		x[c]= std::make_unique<signed short []>(maxdelay);
		ax[c]= std::make_unique<signed short []>(maxdelay);
		ay[c]= std::make_unique<signed short []>(maxdelay);
	}
