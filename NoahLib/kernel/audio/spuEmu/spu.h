...
public:
    ...
    virtual void device_post_load() = 0;
    virtual void device_stop() = 0;
};

class device_sound_interface {
public:
    typedef unsigned int sound_stream_flags;
};
...
protected:
	...
	std::vector<unsigned char> spu_ram;
	reverb *rev;
	unsigned int taddr;
...
public:
	...
	uint16_t read(unsigned long int offset);
	void write(unsigned long int offset, uint16_t data);
